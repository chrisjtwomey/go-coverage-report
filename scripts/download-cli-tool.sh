...
if [[ ${#VERSION} == 40 ]]; then
  ...
  go install -v "github.com/chrisjtwomey/go-coverage-report@$VERSION"
  end_group
  exit 0
fi
...
URL="https://github.com/chrisjtwomey/go-coverage-report/releases/download/${VERSION}/${FILENAME}"
...
if ! [[ "$SHA256SUM" ]] ; then
  ...
  URL="https://github.com/chrisjtwomey/go-coverage-report/releases/download/${VERSION}/checksums.txt"
  cd .github/outputs
